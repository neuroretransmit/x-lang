...
HashMap* init_hashmap()
{
	...
	check(m, "Couldn't allocate HashMap_Map");

	m->data = (HashMapElement*) calloc(INITIAL_SIZE, sizeof(HashMapElement));
	check(m->data, "Couldn't allocate initial 256 elements");
 ...
 */
MapStatus hashmap_rehash(HashMap* in)
{
	int old_size;
	HashMapElement* curr;
	...
	HashMap_Map *m = (HashMap_Map*) in;
	HashMapElement* temp 
		= (HashMapElement*) calloc(2 * m->table_size, sizeof(HashMapElement));

	if (!temp) return MAP_OOM;
	...
	for (int i = 0; i < old_size; i++) {
		...
		status = hashmap_put((HashMap*) m, curr[i].key, curr[i].data);
...
void destroy_hashmap(HashMap* in)
{
	HashMap_Map* m = (HashMap_Map*) in;
	free(m->data);
	free(m);
...
size_t hashmap_length(HashMap* in)
{
	HashMap_Map* m = (HashMap_Map *) in;
